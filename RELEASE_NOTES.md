...
**Public release v2.0** | Internal data build 3.3.1 | August 24, 2026
...
- **Prior-pregnancy count structural zeros corrected.** The public v1.0 exporter (internal build v3.2) converted code 88 ("no prior pregnancy") to missing in `parity`, `prior_perinatal_loss`, and `prior_livebirths`. The corrected rule maps 88 to zero and retains 99 as unknown, restoring 16,054, 16,163, and 16,157 values, respectively. See `ERRATA.md`.
...
*Public release v2.0, produced from internal data build 3.3.1, restores prior-pregnancy structural zeros, standardizes the expanded clean-file schemas and identifiers, and coalesces 2,421 source-proven unified-file ID-alias groups. It also retains the twin-specific placental pathology linkage prepared in March 2026. Internal build 3.0 corrected the WISC VIQ/PIQ column identification and added 51 variables from the 7-year psychological battery. Internal build 3.1 added SAS-program enrichment and a full re-parse of 309 card CSVs. Internal build 3.2, published as public release v1.0, rebuilt the unified and comprehensive wide datasets, merged 20 single-row standalone datasets, moved sparse columns to supplementary side-sheets, removed byte-position duplicate columns, fixed trailing-colon name artifacts, parsed SOCIO7YR, and supplied unified CSV and RDS formats. Processing scripts, validation logs, errata, and checksums are bundled with the data files.*
